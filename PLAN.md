...
- RIA 잔액 = `kiwoom-obil` 로 맵핑 (실제 RIA 계정 없음) // RIA는  `kiwoom-obil`아님!! 새로 추가해야 하는 계좌
  ...
  pension-tracker/                               ← 신규 섹션
    plan/
      "2026-01": {
        ...
        isa_limit:      40000000,  ← ISA 납입 한도(원)
        pension_monthly: 500000,   ← 연금저축 월 납입 계획(원)
        irp_monthly:    100000,    ← IRP 월 납입 계획(원)
        isa_monthly:    0,         ← ISA 월 납입 계획(원)
        voo_annual_limit: 2500000, ← VOO 매도 비과세 한도(원)
    ...
    contributions/
      ...
      isa/  { "2026-03": 0, ... }
    voo/
      "2026": { sold: 0, gain: 0 }   ← 연간 VOO 매도금액 / 양도차익
