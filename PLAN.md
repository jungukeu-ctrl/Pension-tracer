...
| **잔여 공제 한도** | 250만원 - 연간 누적 양도차익 | 이 금액 내에서만 매도 (초과 매도 하지 않음) |
...
연도별 ISA 납입 흐름 (확정):
  ...
  2028년:
    남은 RIA 잔액 → ISA 이전
    이전 완료 후 남은 연간 한도(2,000만원 - 이전금액) 내에서 월 25만원 납입 재개
```
  ...
  pension-tracker/                               ← 신규 섹션
    plan/
      "2026-01": {
        ...
        isa_annual_limit: 20000000, ← ISA 연간 납입 한도(원, 일반형, 1/1~12/31)
...
#### 2-2-1. 분석 차트 상세 (구현 완료 ✅)

| 탭 | 차트 종류 | 설명 |
|----|----------|------|
| **연금자산 추이** | 시계열 라인 | 월별 잔액 추이. 계좌 필터(전체/연금저축/IRP/ISA/RIA). 전체 선택 시 합계선 + 누적 계획납입 점선(월 150만 기준) 표시 |
| **납입 달성률** | 수평 바 | 연도 선택. 연간 목표 / 현시점 계획(경과N개월×월납입) / 실적 3겹 바 비교. 연간 달성률(%) 툴팁 |
| **원금 vs 평가** | 그룹 바 | 기준 월 선택. 계좌별 누적납입원금 vs 평가금액. 수익 녹색/손실 적색. 수익금액·수익률 툴팁 |
| **VOO 현황** | 도넛 | 실현 양도차익 vs 잔여 한도(250만원). 잔여 매도 가능 금액 표시. 한도 초과 시 과세 경고 |

```
차트 구현 스펙:
  라이브러리:  Chart.js 4.4.0 (CDN)
  테마:        다크 (#0f1117 계열), 앱 CSS 변수 연동
  데이터 소스: localStorage (pension-tracker-v1)
  갱신 시점:   저장 버튼 클릭 시 현재 탭 차트 자동 갱신
  반응형:      maintainAspectRatio: false, 높이 고정(260px)

계획납입 기준선 산출 방식 (추이 차트):
  첫 저장 월 기준 상대 계산
  N개월차 누적 = N × (연금저축 125만 + IRP1 25만) = N × 150만원
  ※ ISA 납입은 VOO 미실현 구간이므로 기준선에 미포함
```
